...
import (
	...
	"github.com/chislab/go-ethereum/accounts/abi"
	"github.com/chislab/go-ethereum/accounts/abi/bind"
	"github.com/chislab/go-ethereum/common"
	"github.com/chislab/go-ethereum/common/hexutil"
	"github.com/chislab/go-ethereum/core/types"
	"github.com/chislab/go-ethereum/crypto"
	"github.com/chislab/go-ethereum/ethclient"
)
...
func rawDeploy(gethCli *ethclient.Client) *common.Address {
	...
	rawTx := types.NewContractCreation(uint64(nonce), GetLatestBlockNumber()+100, big.NewInt(0), 4700000, big.NewInt(20000000000), payLoad)
...
func testGetAndSet(gethCli *ethclient.Client, opts *bind.TransactOpts, address *common.Address) {
	...
	r, err := deployedHello.Get(&bind.CallOpts{})
	fmt.Println(r)
	opts.RandomId = big.NewInt(time.Now().Unix())
	opts.BlockLimit = big.NewInt(int64(GetLatestBlockNumber() + 100))
	_, err = deployedHello.Set(opts, "miao")
	if err != nil {
...
func GetLatestBlockNumber() uint64 {
	rspBytes, err := ApplyRPCApi("eth_blockNumber", nil, uint64(1))
	if err != nil {
		return 0
	}
...
func GetReceiptByHash(txHash string) *types.FiscoReceipt {
	rspBytes, err := ApplyRPCApi("eth_getTransactionReceipt", []interface{}{txHash}, uint64(1))
	if err != nil {
		return nil
	}
